...
class DebianPackager(object):
    ...
    def CompileControl(self, tweak_data, repo_settings):
        ...
        if tweak_data['maintainer']['email']:
            control_file += "Maintainer: " + tweak_data['maintainer']['name'] + " <" + tweak_data['maintainer']['email'] + ">\n"
        else:
    ...
    def CompilePackages(self):
        ...
        """
        # TODO: Update DpkgPy to generate DEB files without dependencies (for improved win32 support)
        call(["dpkg-scanpackages", "-m", "."], cwd=self.root + "repo/", stdout=open(self.root + "repo/Packages", "w"))
        # For this, we're going to have to run it and then get the output. From here, we can make a new file.
        shutil.copy(self.root + "repo/Packages", self.root + "repo/Packages2")
        call(["bzip2", "Packages"], cwd=self.root + "repo/")
        call(["mv", "Packages2", "Packages"], cwd=self.root + "repo/")
        call(["xz", "Packages"], cwd=self.root + "repo/")

    def SignRelease(self):
        """
        Signs Release to create Release.gpg. Also adds hash for Packages.bz2 in Release.
        """
        with open(self.root + "repo/Packages.bz2", "rb") as content_file,\
            open(self.root + "repo/Packages.xz", "rb") as content_file_xz:
            bzip_raw = content_file.read()
            bzip_sha256_hash = hashlib.sha256(bzip_raw).hexdigest()
            bzip_size = os.path.getsize(self.root + "repo/Packages.bz2")
            xz_raw = content_file_xz.read()
            xz_sha256_hash = hashlib.sha256(xz_raw).hexdigest()
            xz_size = os.path.getsize(self.root + "repo/Packages.xz")
            with open(self.root + "repo/Release", "a") as text_file:
                ...
                try:
                    if repo_settings['enable_gpg'].lower() == "true":
                        ...
                        key = "Oneopc MobileAPT Repository"  # Most of the time, this is acceptable.
                        call(["gpg", "-abs", "-u", key, "-o", "Release.gpg", "Release"], cwd=self.root + "repo/")
    ...
    def PushToGit(self):
        ...
        call(["git", "commit", "-am", "Repo contents updated Silica v1.1.0"], cwd=self.root)
